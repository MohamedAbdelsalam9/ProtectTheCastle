...
void DrawEnemy(enemy& E, int Ypos)
{
	...
	if (E.getRegion() =='A')
	{
		x= CastleXStrt-E.getDistance()+1;
		y= (CmdHeight/2)-(CastleLength/4)-Ypos;
	}
	else if(E.getRegion() == 'B')
	{
		x= CastleXStrt+CastleWidth+E.getDistance() -1;
		y= (CmdHeight/2)-(CastleLength/4)-Ypos;
	}
	else if(E.getRegion() == 'C')
	{
		x= CastleXStrt+CastleWidth+E.getDistance() -1;
		y= (CmdHeight/2)+(CastleLength/4)+Ypos;
	}
...
void DrawEnemies(vector<enemy*> enemies)
{
	...
	for(int region=1;region<=4;region++)
	{
		...
		for(int distance=((CmdWidth/2)-(CastleWidth/2));distance>1;distance--)
		{
			...
			{	
				if(enemies[i]->getDistance()==distance && enemies[i]->getRegion() == (char)region+'A'-1)
				{
					CountEnemies++;
				}
			}
